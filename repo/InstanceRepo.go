...
func GetInstancesFromDBWithPage(Instances *[]Instance, page string) {
	db := other.ConnectDB()
	defer db.Close()
	p, _ := strconv.Atoi(page)
	get, err := db.Query(fmt.Sprintf("Select * from `instances` order by instance_id LIMIT 5 OFFSET %d", (p-1)*5))
	other.CheckErr(err)
